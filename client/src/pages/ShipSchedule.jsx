...
const ShipSchedule = () => {
  ...
  const fetchSchedules = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/ship-schedule');
      setSchedules(response.data);
      setLoading(false);
    } catch (error) {
      // Fallback data
      setSchedules([
        { id: 1, shipName: "MV Swaraj Dweep", from: "Port Blair", to: "Havelock Island", departure: "06:00 AM", arrival: "08:30 AM", status: "On Time", days: ["Mon", "Wed", "Fri", "Sun"] },
        { id: 2, shipName: "MV Makruzz", from: "Port Blair", to: "Havelock Island", departure: "08:15 AM", arrival: "10:30 AM", status: "On Time", days: ["Daily"] },
        { id: 3, shipName: "MV Coastal Cruise", from: "Havelock Island", to: "Neil Island", departure: "11:00 AM", arrival: "12:00 PM", status: "On Time", days: ["Tue", "Thu", "Sat"] },
        { id: 4, shipName: "MV Green Ocean", from: "Port Blair", to: "Neil Island", departure: "06:30 AM", arrival: "09:00 AM", status: "Delayed 30 min", days: ["Mon", "Wed", "Fri"] },
        { id: 5, shipName: "MV Nautika", from: "Havelock Island", to: "Port Blair", departure: "04:00 PM", arrival: "06:30 PM", status: "On Time", days: ["Daily"] },
      ]);
      setLoading(false);
    }
  };
  ...
  const getStatusStyles = (status) => {
    if (status.includes('On Time')) return { bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-600 dark:text-green-400', icon: FaCheckCircle };
    if (status.includes('Delayed')) return { bg: 'bg-yellow-100 dark:bg-yellow-900/30', text: 'text-yellow-600 dark:text-yellow-400', icon: FaExclamationCircle };
    return { bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-600 dark:text-red-400', icon: FaExclamationCircle };
  };
                  ...
                  <div className="relative mb-6">
                    <div className="flex items-center justify-between">
                      <div className="text-center">
                        ...
                        <p className="text-xs text-gray-500">{schedule.departure}</p>
                      </div>
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-xs text-gray-500">{schedule.arrival}</p>
                      </div>
                    </div>
                  </div>
